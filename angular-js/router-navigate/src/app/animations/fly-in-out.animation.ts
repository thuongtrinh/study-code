...
trigger('flyInOutTrigger', [
  ...
  transition(':enter', [
    ...
    group([
      animate('0.5s 0.1s ease-in', style({ transform: 'translateX(0)' })),
      animate('0.3s 0.1s ease', style({ opacity: 1 }))
    ])
  ]),
  transition(':leave', [
    style({ backgroundColor: '#9DCEFC' }),
    group([
      animate('0.5s ease-out', style({ transform: 'translateX(300%)' })),
      animate('0.3s 0.1s ease', style({ opacity: 0 }))
    ])
  ])
]);
